...
def neural_network_non_lineaire(X_train, y_train, nombre_neurone, n_iter = 100 , learning_rate = 0.001):

    # Initialisation des modules
    mse = MSELoss()
    linear_1 = Linear(X_train.shape[1], nombre_neurone)
    tanh = Tanh()
    linear_2 = Linear(nombre_neurone, y_train.shape[1])
    sigmoide = Sigmoide()
    ...
    for _ in range(n_iter):
        ...
        linear_1.backward_update_gradient(X_train, delta_tan)
        ...
        list_errors.append(np.mean(mse.forward(y_train, res4)))

    return list_errors,linear_1,linear_2
...
datax, datay = gen_arti(data_type = 1 , epsilon = 0.00001)
